# Worker-Code Architecture: Graph RAG and Semantic Indexing

The `worker-code` service is a specialized microservice within the Insightify platform dedicated to codebase analysis. It implements an advanced **Graph RAG (Retrieval-Augmented Generation)** architecture. 

Unlike traditional code analyzers that treat code as flat text and dump it into a Vector Database, `worker-code` parses repositories into an Abstract Syntax Tree (AST), maps structural dependencies into a Neo4j Graph Database, stores raw blobs locally, and uses a LangGraph-orchestrated Text-to-Cypher pipeline for highly accurate intelligence.
...
An intentionally localized, specialized LLM (e.g., Ollama Llama 3.2 running at low temperature) is provided with the user's intent and prompt instructions.
...
- The `CodeStore` reads the disk-based `.txt` files containing the raw python/JS text up to a strict token ceiling (e.g., `max_chars = 3000`).
...
The `memory_manager_agent` compresses the transaction context into a slide-window summary. The `output_assembler` bundles the Cypher code, chart payloads, and the final textual insight into `CodeAnalysisState` resulting payload and returns it seamlessly bridging graph infrastructure securely to the UI workspace.
