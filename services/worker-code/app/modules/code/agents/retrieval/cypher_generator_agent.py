...
Graph Schema:
(f:Function) / (c:Class) nodes have:
  - name, file_path, line_start, line_end
  - architectural_importance (PageRank score, higher is more central)
  - architectural_module_id (Louvain community ID, groups related code)

Relationships:
- (f)-[:CALLS]->(f2)
- (c)-[:HAS_METHOD]->(f)
- (f)-[:SIMILAR_TO]->(f2) (Semantic vector similarity)

STRICT RULES:
1. ALWAYS filter by: WHERE n.source_id = $source_id.
2. For "important" or "core" code, sort by `n.architectural_importance DESC`.
3. For "related" or "modular" code, use `n.architectural_module_id`.
4. Use LIMIT 20 to avoid overwhelming the context.
