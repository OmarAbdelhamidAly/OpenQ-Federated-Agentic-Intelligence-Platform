...
def get_connection_string(state: Dict[str, Any]) -> Optional[str]:
    ...
    1. Mirrored File (CSV/JSON) → returns internal Postgres connection string
    2. Uploaded SQLite file → returns sqlite:///file_path
    3. Remote SQL DB → decrypts and builds a full connection string
    """
    ...
    # Case 2: Uploaded SQLite database file
...
def get_source_type(state: Dict[str, Any]) -> str:
    """Return normalised source type: 'csv', 'json', or 'sql'."""
    raw = (state.get("source_type") or "csv").lower()
    if raw in ("sql", "sqlite", "postgresql", "mysql", "mssql"):
        return "sql"
    if raw == "json":
        return "json"
    return "csv"
