...
class CorrelationInput(BaseModel):
    ...
    records: List[Dict[str, Any]] = Field(..., description="List of dicts from MongoDB aggregation result")
...
def compute_correlation(
    ...
    """Compute a correlation matrix for numeric columns from JSON/MongoDB data."""
    if not records:
        return {"error": "No records provided for correlation."}
    ...
    if columns:
        ...
        numeric_df = df[valid_cols].select_dtypes(include=["int64", "float64", "int32", "float32"])
    else:
