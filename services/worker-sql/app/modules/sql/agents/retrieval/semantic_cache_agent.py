"""Semantic Caching Utility for SQL Worker — Allows saving results to the global cache."""
import structlog
import uuid
...
async def save_semantic_cache(state: AnalysisState) -> Dict[str, Any]:
    """Terminal node that saves the final insight report to the semantic cache."""
    ...
    if not state.get("history") and question and answer and tenant_id:
        try:
            ...
            logger.info("semantic_cache_saved_sql", tenant_id=tenant_id, source_id=source_id)
        except Exception as e:
            logger.warning("semantic_cache_save_failed_sql", error=str(e))
